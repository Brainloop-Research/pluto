...
namespace pluto {
    auto tensor::create(context* const ctx, const std::span<const linear_dim> shape) noexcept -> tensor* {
        ...
        t->m_ctx = ctx;
        constexpr linear_dim scalar_size {sizeof(float)};
        linear_dim size {static_cast<linear_dim>(std::accumulate(shape.begin(), shape.end(), scalar_size, std::multiplies<>{}))};
        ...
        for (linear_dim i {1}; i < max_dims; ++i)
            t->m_strides[i] = t->m_strides[i-1]*t->m_shape[i-1];
        t->m_rank = static_cast<linear_dim>(shape.size());
        return t;
    }
}
